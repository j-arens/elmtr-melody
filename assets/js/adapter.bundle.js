...
const { MELODY_ENV: { pluginsUrl } } = window;
exports.defaultTrack = {
...
Object.defineProperty(exports, "__esModule", { value: true });
const defaults_1 = __webpack_require__(0);
const mergewith = __webpack_require__(6);
const { MELODY_ENV: { pluginsUrl, siteUrl } } = window;
exports.getDownloadUrl = (track, origin) => {
    if (origin === 'external') {
...
exports.assignDuration = (track, duration) => Object.assign(track, { duration });
exports.mergeIntoDefault = track => mergewith({}, defaults_1.defaultTrack, track, (oV, srcV) => srcV === '' ? oV : undefined);
exports.trackLengthToSeconds = (str) => {
    if (!str.includes(':')) {
        return 0;
    }
...
exports.getTrackLengths = (tracks) => __awaiter(this, void 0, void 0, function* () {
    return yield Promise.all(tracks.map(t => {
        if (t.melody_audio_source === 'media-library') {
            const duration = t.melody_wp_media_picker.duration;
            return exports.trackLengthToSeconds(duration);
        }
...
const _2 = __webpack_require__(8);
const throttle = __webpack_require__(12);
const { MELODY, jQuery: $ } = window;
if (MELODY === undefined) {
    console.error('lol adapter cant work error'); // @TODO: make gud error msg
}
$(window).on('melody/init_app', throttle(init, 500));
function init({ config, selector }) {
    return __awaiter(this, void 0, void 0, function* () {
        const { app } = MELODY;
        const store = app.getStore();
        resetApp(store);
        setView(store, config.melody_component_style);
        yield injectTracks(store, config.melody_audio_tracks);
        yield injectCss(store, config);
        app.mount(selector);
    });
}
...
const injectTracks = ({ dispatch }, tracks) => __awaiter(this, void 0, void 0, function* () {
    return dispatch({
        type: 'melody/SET_TRACKS',
        payload: yield _1.prepareTracks(tracks),
    });
});
const injectCss = ({ dispatch }, config) => dispatch({
    type: 'melody/EDIT_CUSTOM_PROPS',
    payload: _2.prepareRules(Object.assign({}, config)),
});
MELODY.adapter = {
    init,
};
...
Object.defineProperty(exports, "__esModule", { value: true });
const defaults_1 = __webpack_require__(0);
const mappers = __webpack_require__(5);
const helpers_1 = __webpack_require__(1);
exports.prepareTracks = (tracks) => __awaiter(this, void 0, void 0, function* () {
    if (!Array.isArray(tracks)) {
        tracks = [];
    }
    const durations = yield helpers_1.getTrackLengths(tracks);
    return tracks
        .map((t, index) => helpers_1.assignDuration(t, durations[index]))
        .map(normalizeTrack);
});
const normalizeTrack = track => {
    let normalized = defaults_1.defaultTrack;
    switch (track.melody_audio_source) {
        case 'external-source': {
            normalized = mappers.external(Object.assign({}, track));
            break;
        }
        case 'media-library': {
            normalized = mappers.mediaPicker(Object.assign({}, track));
            break;
        }
        default: {
            break;
    ...
    return helpers_1.mergeIntoDefault(normalized);
};
...
function external(track) {
    const { melody_track_url, melody_track_artwork, melody_track_artist, melody_track_album, melody_track_title, duration, } = track;
    return {
        source_url: melody_track_url,
        download_url: helpers_1.getDownloadUrl(track, 'external'),
...
function mediaPicker(track) {
    const { duration, melody_wp_media_picker: { album, artwork, artist, title, url, }, } = track;
    return {
        source_url: url,
        download_url: helpers_1.getDownloadUrl(track, 'internal'),
        artwork,
        media_details: {
            artist,
            title,
...
Object.defineProperty(exports, "__esModule", { value: true });
const MiddlewarePipeline_1 = __webpack_require__(9);
const middleware_1 = __webpack_require__(10);
exports.prepareRules = config => Object.keys(config)
    .reduce((rules, key) => {
    const pipeline = new MiddlewarePipeline_1.default;
    pipeline.use(middleware_1.filterRules, middleware_1.assembleBorders, middleware_1.assembleBoxShadows, middleware_1.assemblePaddings, middleware_1.assembleQuantities);
...
class MiddlewarePipeline {
    ...
    process(...args) {
        if (this.repository.size) {
            for (const middleware of this.repository.values()) {
                if (this.stopped) {
                    break;
                }
...
exports.filterRules = (stop, config, props, key) => {
...
exports.assembleBorders = (stop, config, props, key) => {
    switch (true) {
        case key.endsWith('border_width'): {
            const name = helpers_1.getUniqueKeyName(key, 'border_width');
        ...
        case key.endsWith('border_radius'): {
            const name = helpers_1.getUniqueKeyName(key, 'border_radius');
            const { unit, top, right, bottom, left } = config[key];
...
exports.assembleBoxShadows = (stop, config, props, key) => {
    if (key.endsWith('box_shadow_box_shadow')) {
        const name = helpers_1.getUniqueKeyName(key, 'box_shadow_box_shadow');
...
exports.assemblePaddings = (stop, config, props, key) => {
    if (key.endsWith('padding')) {
        const { top, left, right, bottom, unit } = config[key];
        const name = helpers_1.getUniqueKeyName(key, 'padding');
...
exports.assembleQuantities = (stop, config, props, key) => {
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.getUniqueKeyName = (key, common) => key.substring(0, key.indexOf(common));
